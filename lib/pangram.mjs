...
// Used by the local dev server (server.js) and by any host that can run
// web-standard Request/Response handlers (Next.js route handlers, etc.).
//
// Pangram's API sends no CORS headers, so the Word task pane calls one of
// these handlers instead. The API key comes from the request's x-api-key
// header (each user's own key) or, as a fallback, PANGRAM_API_KEY in the
// environment — but on a public host that lets anyone spend those credits.
...
export function resolveKey(req) {
  ...
  const key = fromClient || process.env.PANGRAM_API_KEY;
...
export async function handleStatus(req) {
  const hasServerKey = Boolean(process.env.PANGRAM_API_KEY);
  let models = [];
  let keyOk = null;
  try {
    models = await listModels(resolveKey(req));
    keyOk = true;
  } catch (e) {
...
export async function handleAnalyze(req) {
  try {
